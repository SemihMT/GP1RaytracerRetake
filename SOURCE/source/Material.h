...
#pragma region Material COOK TORRENCE
	...
	public:
		...
		ColorRGB Shade(const HitRecord& hitRecord = {}, const Vector3& l = {}, const Vector3& v = {}) override
		{
			...
			ColorRGB kd{ ColorRGB{1,1,1} - BRDF::FresnelFunction_Schlick(halfVector,v,f0) };
			...
			//Calculate Fresnel (F), Normal Distribution (D) & Geometry (G)
			...
			float    G = BRDF::GeometryFunction_Smith(hitRecord.normal, v.Normalized(), l.Normalized(), m_Roughness);

			//Calculate specular => Cook-Torrance -> (DFG)/4(dot(v,n)dot(l,n))
			ColorRGB DFG{ D * F * G };
			const float denominator{ 1.f/(4.f * (Vector3::Dot(v, hitRecord.normal) * Vector3::Dot(l, hitRecord.normal))) };
