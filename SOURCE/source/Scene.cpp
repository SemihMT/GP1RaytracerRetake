...
#pragma region SCENE W3
	void Scene_W3_TestScene::Initialize()
	{
		...
		const auto matCookTorrence_Blue = AddMaterial(new Material_CookTorrence({ 1,0,0 }, 0, 0.6f));
		//Spheres
		AddSphere({ -.75f,1.f,.0f }, 1.f, matLambert_Red);
		AddSphere({ .75f,1.f,.0f }, 1.f, matCookTorrence_Blue);
	...
	void Scene_W4_ExtraScene::Initialize()
	{
		...
		const auto matLambert_Green = AddMaterial(new Material_Lambert({0.2f,0.8f,0.1f }, 1.f));
		const auto matLambert_Magenta = AddMaterial(new Material_Lambert(colors::Magenta, 1.f));
		const auto matLambert_Blue = AddMaterial(new Material_Lambert(colors::Blue, 1.f));

		
		AddPlane({ .0f,-1.f,0.0f }, { 0.f,1.f,0.f }, matLambert_Green); //Bottom
		AddPlane({ .0f,0.f,10.0f }, { 0.f,0.f,-1.f }, matLambert_Blue); //Bottom
		


		pMesh = AddTriangleMesh(TriangleCullMode::BackFaceCulling, matLambert_Magenta);
		Utils::ParseOBJ("Resources/Test.obj"
			, pMesh->positions
			, pMesh->normals
			, pMesh->indices);
	...
	void Scene_W4_ExtraScene::Update(Timer* pTimer)
	{
		...
		pMesh->RotateY(yawAngle);
		pMesh->Translate({0, 3 + sinf(yawAngle),0});
		pMesh->UpdateTransforms();
		pMesh->UpdateTriangleList();
	}
